...
建议同时补两组对照数据：
- `50ms`：中高频切换，对照 `20ms` 极限压测，便于观察优化趋势是否稳定
- `150ms`：更接近用户手动点击 / 常规切换节奏，用于评估真实体验收益
...
```bash
./scripts/tab-switch-stress.sh 20 150 0.5
```
...
`Settings` AppKit 化阶段结果（2026-03-30 最新）：

| 阶段 | 20ms CPU(avg/peak) | 20ms RSS(avg/peak) | 150ms CPU(avg/peak) | 150ms RSS(avg/peak) |
| --- | --- | --- | --- | --- |
| 全真实页面基线（SwiftUI） | 69.61% / 79.40% | 83.05MB / 102.31MB | - | - |
| 仅“快捷键”卡 AppKit 化 | 54.45% / 72.50% | 74.23MB / 89.12MB | 22.60% / 65.00% | 88.87MB / 94.16MB |
| `Settings` 全卡片 AppKit 化 | 40.90% / 55.00% | 81.89MB / 91.11MB | 14.52% / 48.50% | 74.63MB / 79.91MB |
...
- 在不改现有“满页面表单”视觉设计的前提下，优先把重卡片迁移为 AppKit 岛，已验证能显著降低 tab 切换 CPU。
- 当前下一步可继续验证“外层双列 `ScrollView` 宿主 AppKit 化”是否能进一步降低 `20ms` 高频切换成本。
