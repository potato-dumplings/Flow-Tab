...
- [ ] 1) 锁定基线 A  
恢复到 A 行为（Home+Logs+Settings 全保活），并固定 20ms / 50ms 两组压测基线，各执行 3 轮记录均值与峰值。
...
- [ ] 8) 压测验收门槛（相对 A 不退化）  
20ms 与 50ms 两组场景下，CPU / RSS 的 avg 与 peak 均不得劣于 A 基线。

- [ ] 9) 发布策略  
增加 feature flag（如 `stable_nav_pipeline_v2`），先灰度对比 A，再切默认开启。

- [ ] **10) 强调项：严格“一步一改一测”**  
每次只改一个点，改完立即压测并记录；若任一指标退化，立刻回滚“当步改动”，禁止多点叠加后再统一排查。
