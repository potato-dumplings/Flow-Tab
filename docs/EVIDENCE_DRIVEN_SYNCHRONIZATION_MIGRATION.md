...
| SYNC-037D | `runtime-topology-pressure.sh` signal/exit cleanup and LaunchServices-launched FlowTab/Space fixture processes | An interrupted xcodebuild tree prevents XCTest teardown, while the exact FlowTab target and fixture application are already reparented to launchd and therefore outlive the runner tree. Cleanup has no owner for these independent identities. | Before starting the UI wrapper, capture exact pre-existing target and configured fixture identities. On interruption, use the frozen target manifest/launch receipt and resolved fixture workflow to identify only post-request PID/start/path/bundle identities, request termination, and require exact final absence readback. Register any workspace observer before the launch trigger and close missed-event races with initial and final readback. The pressure invocation owns observation, cancellation, and cleanup; named watchdogs only bound failure and report every remaining identity. | H independent application lifecycle and user-process safety; deterministic baseline/post-request/duplicate/PID-reuse/cancel/watchdog rules, real interrupted fixture UI Pressure, Process/Tooling. | planned; discovered by SYNC-037C interrupted validation |
