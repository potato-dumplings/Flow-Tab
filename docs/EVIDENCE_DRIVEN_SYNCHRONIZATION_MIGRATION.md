...
| SYNC-034 | `FlowTabUITests+Support.swift`, `+WorkflowWindowObservation.swift`, `+SpaceFixtureApp.swift`, `+ScrollingSupport.swift`, `+StatusItem.swift`, and fixture assertion helpers; shared UI condition loops | RunLoop cadence advances drive XCUI/CG/AX/process predicate observation. Conditional observation. | Centralize named UI observation cadence and watchdog diagnostics, check immediately, use `waitForExistence`/predicate expectations where possible, and keep exact CG/AX/window/process readback as the sole Oracle. XCTest case lifetime owns the wait. | H test infrastructure; affected UI suites, Process/Tooling. | in progress; SYNC-034A–SYNC-034AJ and SYNC-034AL–SYNC-034AO completed, SYNC-034AK validation pending, with remaining owners split into later slices |
...
- UI-test infrastructure regression and Pressure: six focused tests cover an
  ...
  cancel/restart/stale-generation iterations with duplicate delivery, and
  watchdog final-projection diagnostics. All six compile in the complete
  UI-test target.
- UI environment blocker: the final read-only preflight finds `coreautha`
  active. The same-session UI owner attempt previously timed out while enabling
  automation mode. Owner/Pressure execution and the affected multi-app
  window-card isolation paths remain open, and the Goal remains active.
- Process/Tooling: the fresh canonical build at
