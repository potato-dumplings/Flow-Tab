...
| SYNC-034 | `FlowTabUITests+Support.swift`, `+WorkflowWindowObservation.swift`, `+SpaceFixtureApp.swift`, `+ScrollingSupport.swift`, `+StatusItem.swift`, and fixture assertion helpers; shared UI condition loops | RunLoop cadence advances drive XCUI/CG/AX/process predicate observation. Conditional observation. | Centralize named UI observation cadence and watchdog diagnostics, check immediately, use `waitForExistence`/predicate expectations where possible, and keep exact CG/AX/window/process readback as the sole Oracle. XCTest case lifetime owns the wait. | H test infrastructure; affected UI suites, Process/Tooling. | in progress; SYNC-034A–SYNC-034AH and SYNC-034AL–SYNC-034AO completed, SYNC-034AI–SYNC-034AK validation pending, with remaining owners split into later slices |
...
- UI-test infrastructure regression and Pressure: five focused tests cover
  ...
  delivery, and watchdog final-title/raw-projection diagnostics. The complete
  UI-test target compiles the owner, suite, and affected Option+Tab and
  Control+Tab clean/noisy window-state paths.
- UI environment blocker: the post-build read-only process check finds
  `coreautha` and `SecurityAgent` active. Owner, Pressure, and affected
  real-fixture UI execution remain open, and the Goal remains active.
...
- UI-test infrastructure regression and Pressure: seven focused tests cover
  ...
  cancel/restart/stale-generation iterations with duplicate delivery, and
  watchdog final-projection diagnostics. The base six-test suite passed 6/6
  with zero failures in 2.429 seconds at
  `.build-local/evidence-driven-sync/SYNC-034AJ/owner-attempt-001`. The final
  exact-count extension compiles in the complete UI-test target.
- UI environment blocker: the final post-build read-only process check finds
  `coreautha` active. Final seven-test owner/Pressure execution and affected
  multi-app, Control+Tab, and Option+Tab clean/noisy real-fixture UI execution
