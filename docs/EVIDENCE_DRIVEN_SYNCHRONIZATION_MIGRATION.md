...
| SYNC-034 | `FlowTabUITests+Support.swift`, `+WorkflowWindowObservation.swift`, `+SpaceFixtureApp.swift`, `+ScrollingSupport.swift`, `+StatusItem.swift`, and fixture assertion helpers; shared UI condition loops | RunLoop cadence advances drive XCUI/CG/AX/process predicate observation. Conditional observation. | Centralize named UI observation cadence and watchdog diagnostics, check immediately, use `waitForExistence`/predicate expectations where possible, and keep exact CG/AX/window/process readback as the sole Oracle. XCTest case lifetime owns the wait. | H test infrastructure; affected UI suites, Process/Tooling. | in progress; SYNC-034A–SYNC-034AG and SYNC-034AL–SYNC-034AO completed, SYNC-034AH–SYNC-034AK validation pending, with remaining owners split into later slices |
...
- UI-test infrastructure regression and Pressure: six focused tests cover
  ...
  final parsed entries. The complete UI-test target compiles the owner, suite,
  and all affected Option+Tab, Control+Tab, clean, and noisy fixture paths.
