...
- UI-test infrastructure regression and Pressure: five focused tests cover
  ...
  path.
- UI environment blocker: the pre-execution read-only process check still
  finds `coreautha` active after the SYNC-034AA automation-mode initialization
  timeout. Owner, Pressure, and affected physical Control+Tab UI execution
