...
- UI-test infrastructure regression and Pressure: the shared suite now has six
  ...
  paths.
- UI environment blocker: the post-build read-only process check finds both
  `coreautha` and `SecurityAgent` active. Owner, Pressure, and affected pointer
  UI execution remain open, and the Goal remains active.
- Process/Tooling: the canonical build at
