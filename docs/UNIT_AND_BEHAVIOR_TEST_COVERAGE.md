...
- `testRuntimeSnapshotProviderUsesUITestMockDatasetWhenLaunchFlagEnabled`
  场景：UI test 启动参数要求使用 Mock Runtime 数据集。
  步骤：注入 `--flowtab-ui-mock-runtime` 后读取 `RuntimeSnapshotProvider`。
  验证：快照中的应用数量、首尾应用、窗口数、首页摘要与上下文映射都与 Mock 数据集一致。

- `testRuntimeSnapshotProviderRealPathWithoutAccessibilityBuildsConsistentSnapshot`
  场景：真实运行路径下没有无障碍权限。
  步骤：关闭无障碍权限、开启“在 Command-Tab 中显示”和“隐藏最小化应用”，再读取快照与首页摘要。
  验证：快照会只保留应用层信息、窗口数组为空、上下文数量与应用数量一致，首页摘要也同步为零窗口。
