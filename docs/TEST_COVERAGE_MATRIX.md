...
2. `P1` Settings window behavior E2E: change auto-enter delay or related toggles and verify the switcher behavior changes through the visible workflow.
3. `P1` Appearance E2E: change theme or language and assert visible UI output changes after the setting is applied.
4. `P1` Real workflow activation: click a Home window row backed by a fixture app and assert the exact fixture window becomes frontmost.
5. `P1` Real workflow app search: search for a real fixture app in app scope and assert the target fixture app becomes frontmost.
6. `P2` Real workflow edge cases: duplicate titles, visually identical windows, non-ASCII titles, punctuation, whitespace, and long titles through the AX/runtime path.
