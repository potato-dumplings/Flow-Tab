...
| Runtime activation and recovery | Partial: deterministic target/fallback helpers | Strong: app activation, window activation, minimized restore, missing-window fallback, recovery retries | Strong: real window-scope search activation and real Home window-row activation are covered | Partial when real fixture workflows are used | Representative real activation paths are covered; pathological duplicate-title and identical-window cases remain tracked under runtime snapshot/window-record gaps. |
...
1. `P1` Real workflow app search: search for a real fixture app in app scope and assert the target fixture app becomes frontmost.
2. `P2` Real workflow edge cases: duplicate titles, visually identical windows, non-ASCII titles, punctuation, whitespace, and long titles through the AX/runtime path.
