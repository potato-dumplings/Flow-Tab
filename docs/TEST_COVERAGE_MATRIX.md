...
1. `P1` Real workflow activation: click a Home window row backed by a fixture app and assert the exact fixture window becomes frontmost.
2. `P1` Real workflow app search: search for a real fixture app in app scope and assert the target fixture app becomes frontmost.
3. `P2` Real workflow edge cases: duplicate titles, visually identical windows, non-ASCII titles, punctuation, whitespace, and long titles through the AX/runtime path.
