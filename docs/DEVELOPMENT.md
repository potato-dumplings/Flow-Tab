...
- 在独立窗口面板放开应用内窗口按键集合中的任一按键：确认当前高亮窗口，并将该窗口切到前台。
...
- 可在设置页直接按键录入：
  ...
  - 主切换按键：默认 `Tab`。
  - 结束应用按键：默认 `Q`。
  - 应用内窗口：默认 `Control + Tab`。
  - 应用内反向修饰键：默认 `Shift`。
- 六项都是非空按键集合，可包含任意识别到的修饰键、普通键与功能键，数量不设上限。录制会累计同时按住的全部按键，并在整组完全松开后提交。
- 主快捷键分组的前四项之间两两不可使用相同按键；应用内分组的两项之间不可使用相同按键。
- 两个分组之间可复用部分按键；主正向、主反向、结束应用、应用内正向、应用内反向这五个最终动作组合必须互不相同。
- 违反同键或最终组合规则时，当前字段显示红字冲突提示，候选值被拒绝并保留原值。
- 主切换与应用内切换的正反组合都能由“修饰键 + 单个普通键”表达时沿用 Carbon 热键注册；任一路径需要任意按键集合监听时，两条切换路径统一使用辅助功能授权下的 CG Session 事件监听器，并按完整集合精确匹配，避免较短组合抢占复用按键。组合内的按键事件仍会正常传递给当前应用；授权后回到 FlowTab 会自动重试监听。
...
- 主正向组合为“主修饰键 ∪ 主切换按键”，主反向组合为“主修饰键 ∪ 反向修饰键 ∪ 主切换按键”，结束应用组合为“主修饰键 ∪ 结束应用按键”。应用内正向组合即“应用内窗口”，应用内反向组合为“应用内窗口 ∪ 应用内反向修饰键”。
...
| 面板内 | `Esc` | 取消本次切换 |
| 面板内 | 释放主按键集合中的任一按键 | 根据当前面板确认并进入应用或窗口 |

说明：
- 结束应用快捷键是“面板会话内动作”，即先通过主切换快捷键进入会话，再按结束应用快捷键。
- 释放主按键集合后的进入行为与“2.2) 放开主按键集合时的进入规则”一致。
