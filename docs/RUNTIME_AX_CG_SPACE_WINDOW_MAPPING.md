...
- `RuntimeAppWindowRepairPayload` 只保留为 app-window repair 兼容 wrapper，由 `RuntimeCurrentAppWindowPayload` 包装生成；provider 只把采样事实转换为 `RuntimeAppWindowProjectionSeed`，不再私有维护 candidate/context/summary projection assembly，也不再把 repair payload 当作 current-app projection 的上游。
