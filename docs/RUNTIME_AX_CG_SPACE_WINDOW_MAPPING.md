...
- `RuntimeProjectionPayloads.swift` 现在承载 runtime-owned projection payload 与 deterministic assembly 数据类型：`RuntimeFullRepairProjectionPayload`、`RuntimeAppContext`、`RuntimeAppWindowProjectionSeed`、`RuntimeCurrentAppWindowPayload` 与 `RuntimeFullRepairProjectionAssembly*`。`RuntimeSnapshotProvider.FullRepairProjectionAssembly*` 仅作为迁移期 typealias / 测试兼容入口；provider builder 文件不再拥有这些 payload/assembly 类型定义。
- provider full repair builder 现在也通过 `WindowListEntry.projectionSeed(lastActiveAt:)` 进入 `RuntimeCurrentAppWindowPayload` 组装 app-switcher candidate 与 `RuntimeAppContext`，不再在 full repair loop 内手写另一套 candidate/context projection assembly；`fullRepairProjectionPayload()` 与其 private full-repair `collectWindowData(for:)` 聚合入口仍位于 `RuntimeSnapshotProvider+AppProjectionBuilders.swift` 迁移桥内，provider core 文件只保留底层 CG/AX/Space 采样与窗口事实基础设施。
...
- `FlowTabPriorityCoverageTests.testRuntimeSnapshotProviderFullRepairProjectionAssemblySelectsPrimaryRowsAndFiltersMinimizedOnlyApps` 与 `testRuntimeSnapshotProviderFullRepairProjectionAssemblyIncludesMinimizedAppsWhenFilterDisabledAndUsesFallbackGroup` 证明 provider full-repair assembly 的 deterministic testing seam 已迁到 `FullRepairProjectionAssembly*` / `assembleFullRepairProjectionRowsForTesting`，不再暴露 snapshot-shaped assembly API。
