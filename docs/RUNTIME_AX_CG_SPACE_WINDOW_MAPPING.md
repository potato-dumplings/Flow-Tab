...
- Phase 5 本轮 P0 继续收窄 WindowRecord affected-target scope ownership：`RuntimeWindowMappingState.affectedCGWindowIDsByPID(...)` 现在拥有 scoped repair affected CGWindowID 从 current CG facts 与 WindowRecord history 合并成 pid->affected-window scope 的规则；`RuntimeProjectionRepairProvider+Reconciliation.swift` 只消费该 scope，再用 app-directory fact source 解析 appID 和组装 target DTO，不再内联 current-CG/window-record 双路径分组。当前这仍是 repair-provider reconciliation 输入，不是长期 WindowRecord/app-directory/topology 直接 rebuild projection；Search freshness contract 不变，barrier 未成功提交新 generation 时仍必须暴露为 degraded/stale committed result。
