...
- scoped repair affected-target derivation 仍由 `RuntimeProjectionRepairProvider+Reconciliation.swift` 组装；`RuntimeAppWindowReconciliationResult` 现在定义在 `RuntimeProjectionRepairProvider.swift`，与返回它的 `RuntimeProjectionRepairProviding` contract 同层。`RuntimeSnapshotProvider+Reconciliation.swift` 只保留 Space topology / verified focus evidence 写回与 WindowRecord 标记，不再承载 service-facing projection repair result shape。
...
- Phase 5 本轮 P0 继续收窄 window-list fact diagnostics vocabulary：`RuntimeSnapshotProvider+CGWindowSupplement.swift` 的 private formatter helpers 已从 `runtimeSnapshot*` 改为 `runtimeWindowList*`，并把 `logChromeLikeTopologySnapshot(...)` 改为 `logChromeLikeTopologyFacts(...)`。这些 diagnostics 仍保留在底层 `.snapshot` log category，因为它们记录 CG/AX/Space fact sampling 过程，但 helper/API 名不再暗示 surface 可以读取 snapshot-shaped runtime state。P1 同轮只更新 provider callsite；P2 不新增 UI/E2E 或 pressure proof，因为日志字段和值、sampling、WindowRecord mapping、surface behavior、activation 与 Search 架构都未改变。Required behavior coverage: `testRuntimeSnapshotProviderWindowEntriesCarrySpaceEvidence`、`testRuntimeSnapshotProviderWindowListFiltersFullscreenSiblingArtifactsAroundNoisyWindows`、`testRuntimeSnapshotProviderWindowListFiltersDuplicateFullscreenGeometryHosts` 已通过 `run-flowtabtests-local.sh` targeted run，并重新编译 `RuntimeSnapshotProvider+CGWindowSupplement.swift`。Search contract 不变：freshness barrier 未成功提交新 generation 时仍只能暴露 degraded/stale committed result 与 dirty/freshness metadata，不能命名为 fresh、complete、latest 或 current-generation committed。
