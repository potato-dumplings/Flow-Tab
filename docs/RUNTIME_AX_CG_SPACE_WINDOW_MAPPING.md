...
- [ ] sticky 与 `space-backed` 条目的缺证删除已经具备 record-level grace，window-layer 也已能输出 in-grace retained records；但这些条目的真实 `Space` 恢复、私有 `CGWindowID` 激活与提交后 exact 重新学习仍缺生产路径和 UI/E2E 证明。
