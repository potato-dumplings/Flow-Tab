...
- provider full repair builder 现在也通过 `WindowListEntry.projectionSeed(lastActiveAt:)` 进入 `RuntimeCurrentAppWindowPayload` 组装 app-switcher candidate 与 `RuntimeAppContext`，不再在 full repair loop 内手写另一套 candidate/context projection assembly。
