...
- full repair fallback target / cancellation 还未完整建模；Search committed/staging index 已在 Phase 4 推进。
...
- P2 待补：full repair 被 high-priority scoped repair 越过或取消、真实 topology UI/E2E 与 pressure proof。
...
- Switcher session-start background full snapshot 已降级为 runtime-owned maintenance request；scheduler priority/coalescing/promoted-backoff P1 已落地，Search freshness barrier priority 与 selected/current app-window priority 已进入 runtime coordinator，full repair fallback target 与 cancellation 仍需补齐。
