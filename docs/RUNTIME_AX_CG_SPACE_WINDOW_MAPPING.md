...
- [ ] sticky 与 `space-backed` 条目的缺证删除已经具备 record-level grace，window-layer 也已能输出 in-grace retained records；activation verified-focus 也已触发 shared runtime 局部回拉入口，registry-resolvable focused AX readback 会直接写回 CG-first exact WindowRecord evidence。代表性 fullscreen/off-space `CGWindowID` 激活已由 UI/E2E 覆盖，但这些条目的提交后 exact 重新学习仍缺生产 UI/E2E 证明。
- [ ] 公开唯一匹配已使用标题、frame、focused/main 前台候选与 minimized/offscreen 候选作为 public assignment 规则。当前仍未完成的是：这些 public AX state tie-breaker 还缺真实多窗口生产拓扑 UI/E2E 覆盖；此外它们只收窄已由标题/frame 建立的候选集合，不替代私有 exact bridge 或 Space topology recovery。
