...
- `RuntimeProjectionRepairProvider` 不再保留底层 fact provider field；初始化时构造的默认 `RuntimeSnapshotProvider` 只被注入 `RuntimeProjectionRepairFactSource`，由 fact source 私有持有 `RuntimeProjectionRepairFactProviding`。concrete repair facade 只持有 `RuntimeWindowRecordStore`、`RuntimeReconciliationCoordinator` 与 `RuntimeProjectionRepairFactSource`，避免后续 repair-provider implementation 又绕开 fact-source contract 直接调用 bottom CG/AX/Space sampling。
