...
- Phase 6 本轮 P0 继续删除 production direct termination pruning 入口：`RuntimeReadModelStore.markAppTerminated(...)` 已移除，store 不再提供同步剪 app-switcher/Home/current/Search projection cache 的 termination API。normal termination 只能通过 `markAppTerminatedForMainTableProjection(...)` 更新 app directory/current-app/dirty metadata，再由 `RuntimeProjectionService` 通过 repair-provider cleanup + main-table builder 预提交 app-switcher/Home projection cache；committed Search index 不被 termination metadata path 剪枝或刷新。Required behavior coverage 更新为 `testRuntimeReadModelStoreMarksTerminatedAppForMainTableProjectionWithoutRefreshingSearch` 与 same-app multi-PID termination coverage，证明 old committed Search rows 保持 degraded/stale committed，直到 bounded freshness barrier 成功提交新 generation。P2 不新增 UI/E2E 或 pressure proof，因为 surface sequence、activation oracle、Search barrier bound、CG/AX/Space sampling cadence 与 repeated hot-path shape 未改变。Remaining gaps: 真实 committed-index Search UI proof、真实 topology UI/pressure proof 继续跟踪。
