...
- [ ] 在提交流程中补齐 `Space` 恢复与私有 `CGWindowID` 激活链路：先切到目标 Space，再恢复 AX；必要时按 `CGWindowID` 做私有激活，成功后回读 `AXFocusedWindow` 并重新学习 exact binding。当前 activation verified-focus 回调已向 shared `RuntimeSnapshotService.signalWindowFocusVerified(...)` 发信号，并以 `.activationVerified` dirty reason 进入 `RuntimeReconciliationCoordinator` 的 app-local pullback 路径；代表性 Option+Tab fullscreen sibling UI 用例已证明真实 `CGWindowID` 从 fullscreen Space 切到 normal sibling 再回到 fullscreen sibling，但提交后回读 `AXFocusedWindow` 并重新学习 exact binding 的独立 UI/E2E 证明仍未完成。
