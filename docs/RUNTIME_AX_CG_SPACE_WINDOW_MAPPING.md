...
- `RuntimeWindowRecordStore` 现在也拥有 verified-focus / AX-destroyed 面向 WindowRecord table 的 evidence entry：`recordWindowFocusVerification(...)` 包住 focused AX/CG readback exact evidence seeding，`clearDestroyedAXAttachment(...)` 包住 destroyed AX attachment downgrade 与 affected `CGWindowID` return。`RuntimeProjectionRepairProvider+Reconciliation.swift` 只调用 store semantic methods 并调度 coordinator，不再把 raw `mappingStatesByPID` inout 传给 evidence helper。Search freshness contract 不变：barrier 未成功提交新 generation 前仍只能暴露 degraded/stale committed result。
