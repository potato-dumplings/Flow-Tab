...
- [ ] `window-layer` 当前会输出 sticky 与 `space-backed`/CG-only 条目，并已停止让同一 `spaceIDs` 下的不同 CG-only `CGWindowID` 互相去重；只有同 Space 的 sticky 记录会压掉较弱的非 sticky 候选，以保留 `CGWindowID` 主锚点。当前仍未保证每个输出条目都具备真实提交恢复路径，因此“只展示可提交条目”仍未完全达成。
...
- [ ] 公开唯一匹配当前主要依赖标题与 frame；focused、main、最小化状态已由 `RuntimeAXWindowState` 进入采集、WindowRecord 当前 attachment 与 diagnostics，但尚未作为 public unique matching 的系统性判定或 tie-breaker 参与匹配决策。
