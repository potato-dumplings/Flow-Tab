...
- Phase 6 已开始把正常 app-only projection generation 从 repair payload 推向 runtime 主表：`RuntimeReadModelStore.readAppSwitcherProjection()` 与 `readHomeSummaryProjection()` 在 committed projection cache 缺失时，可从 `RuntimeAppDirectoryState` 派生 app identity/group/order、empty windows/context 与 Home app summaries，并携带同一 generation/freshness/dirty metadata。该 derived projection 不读取 CG/AX/Space，不提交 Search index；因为它没有成功提交新的 app-switcher/Home 主表 projection generation，即使 app directory 本身 clean/complete，也只能作为 readable degraded/stale migration bridge，不能被称为 fresh、complete 或 latest。只有 `commitMainTableAppSwitcherProjectionPayload(...)` 成功提交覆盖当前主表事实的 projection cache 后，对应 app-switcher/Home summary projection 才能进入 complete 状态。
...
- app-switcher maintenance 现在也可从主表整表预提交 projection cache：`RuntimeMainTableProjectionBuilding.appSwitcherProjectionPayloadFromMainTables(...)` 只读取 `RuntimeReadModelStore` 已维护的 app directory entries 与 `RuntimeWindowRecordStore.projectedWindowEntries(...)`，生成 `RuntimeAppSwitcherProjectionPayload` 后由 `RuntimeReadModelStore.commitMainTableAppSwitcherProjectionPayload(...)` 写入 app-switcher 与 Home summary projection cache。该 payload/store API 不携带也不写入 app directory evidence，不接受 full-repair sampled rows 作为 directory source；full repair 只能先通过 `commitFullRepairAppDirectoryEvidence(...)` 补主表事实，再由主表 builder 生成 projection。payload 还携带 `hasCompleteWindowCoverage`：只有每个 projected app 都有 WindowRecord-derived window rows 与 matching context coverage 时，store 才能把 committed app-switcher/Home projection 标为 complete；app-directory-only / no-AX / zero-window payload 仍只是 readable degraded/stale projection。`RuntimeReadModelDiagnostics` 同步区分 readable projection 与 complete projection，scheduler 只把 complete projection 当作可跳过 full-repair fallback 的前提；derived 或 incomplete projection 可读，但仍会被视为需要 repair/fallback 的 stale 状态。该 path 不调用底层 CG/AX/Space fact source，不提交 Search index；dirty/pending repair 或 incomplete WindowRecord coverage 存在时只能是 degraded/stale committed projection，不能命名为 fresh、complete、latest 或 current-generation committed。
...
- Search freshness barrier 现在只能从 runtime-owned committed app-switcher projection cache 提交 `committedSearchIndex`：`RuntimeProjectionService.requestSearchIndexFreshnessBarrier(...)` 会先尝试通过 main-table app-switcher projection maintenance 刷新 projection cache，随后把 scoped repaired current-app outcome 降级为 directory evidence/appID/pid trigger，并通过 main-table current-app builder 更新 projection cache；只有在无 deferred/pending repair、projection cache `sourceGeneration` 覆盖当前 runtime generation 且该 cache 已由 complete WindowRecord/context coverage 标记为 complete 时，`RuntimeReadModelStore.commitSearchFreshnessBarrierFromProjectionCache(...)` 才会构建并原子提交新的 Search index。service/store 不再保留 repaired-payload `commitSearchFreshnessBarrierPayloads(...)`、production `stagingSearchIndex`、普通 app-switcher direct commit API 或任何隐式 Search commit 成功路径；stale projection cache、repair 中间态、app-directory-only / no-AX zero-window payload 和任何 partial/staging 数据都不能进入 latest/fresh Search 结果态。barrier 未成功提交新 generation 时仍只能暴露 last committed index 的 degraded/stale committed result 与 dirty/freshness metadata，或 missing committed index。
- Search committed read 现在保留 `committedSearchIndex` 自身的 `sourceGeneration`，只叠加当前 dirty/pending metadata；`RuntimeReadModelStore.readCommittedSearchIndexForSearch()` 只有在 committed index 的 `sourceGeneration == RuntimeReadModelStore.generation` 且无 dirty 时才返回 `committedGenerationValidated` / `committedGenerationResult`。如果 current-app/app-switcher/Home projection repair 清掉 dirty、普通 app-switcher projection commit 清掉 dirty，或 app termination 只更新主表 metadata / 后续 projection cache，但没有通过 freshness barrier 原子提交新的 Search generation，Search 仍必须返回 `degradedStaleCommittedResult`，不能因为 dirty 已清空、projection cache 可读或 terminated-app metadata 已更新而被命名为 fresh/complete/latest/current-generation committed。行为覆盖现在显式断言 dirty 已清但 Search generation 未推进时的 raw readiness/result-state label 只能是 degraded/stale committed，且不能包含 fresh、complete 或 latest。
- Home detail projection read 现在也归 Runtime read-model 边界：`RuntimeProjectionServing.readHomeAppDetailProjection(appID:)` / `RuntimeReadModelStore.readHomeAppDetailProjection(appID:)` 只从 current-app projection cache 返回 `RuntimeHomeAppDetailProjection`，Home feature 只读这个 projection API，不再在 surface 层读取 current-app/app-switcher projection 后自行拼 detail，也不再由 read-model 从 app-switcher projection cache/context 兼容派生 detail。`RuntimeHomeAppDetailProjection` 现在携带同源 `RuntimeProjectionFreshness`；read-model 在读取时会叠加当前 dirty app/pid/CG/pending repair metadata，因此 Home detail 可以是可读但 stale/degraded 的 committed projection，而不是无 metadata payload。普通 app-window dirty signal 可以先从 app directory + WindowRecord 主表预提交 current-app projection cache，因此 Home detail 缺 projection 时的下一次 refresh 可读到主表生成的 dirty/stale Home detail projection，而不是等待 repaired payload rows。该 read 不触发 CG/AX/Space sampling，也不提交 Search index。
- Home active/appear refresh scheduling 不再在 surface 层读取 `NSWorkspace.runningApplications` 或维护 `cachedRunningAppSignature`。`HomeRuntimeProjectionRefreshPolicy` 只根据 Home projection 是否已有 summary 且 initial window-count loading 已结束来决定是否请求 runtime projection refresh；缺 projection 时仍通过 `HomeRuntimeRefreshReader` 保留当前 committed UI state 并发送 runtime maintenance/dirty signal。Home 因此不再持有 app directory topology state，也不会用 surface-local running-app diff 触发正常 projection ownership。Search 语义不变：Home projection 可读或 refresh 被请求，都不能让 Search 进入 fresh/complete/latest/current-generation 结果态；freshness barrier 未成功提交新 generation 时仍只能是 degraded/stale committed result 或 missing committed index。
...
- current-app repair / lifecycle upsert evidence 不再能用缺失 rank 的 `RuntimeAppDirectoryEntry` 擦掉既有 app directory rank：`RuntimeAppDirectoryState.upsert(...)` 会在 incoming evidence 没有 `activationRank` 时保留同 PID committed entry 的 rank，而 focused current-app repair evidence 现在用 `RuntimeWindowListFacts.rankByPID` 组装 `RuntimeCurrentAppRepairEvidence.appDirectoryEntries`。这保持 rank/recency ownership 在 Runtime app directory 主表里，而不是让 scoped repair fallback 把 normal projection ordering 拉回无 rank 状态；若 incoming evidence 显式携带新 rank，仍会推进 `appLifecycle` generation 并由主表 projection 反映。Search freshness 语义不变：freshness barrier 成功提交覆盖新 generation 的 committed index 之前，只能返回 last committed degraded/stale result 或 missing committed index，不能把这类 rank-preserving projection read 命名为 fresh、complete、latest/current-generation 或最新完整搜索结果。
...
- app termination lifecycle signal 现在在 committed app directory state 已初始化时以 directory entries 作为 authoritative PID scope：terminated PID 只会从 app directory state 中剪掉，未知/已删除 PID 不再回退到旧 app-switcher/Home/Search cache 推断。若同 appID 仍有 surviving PID，app-switcher/Home projection 会保留 grouped app 并标记 dirty/stale/pending repair；若最后一个 PID 被剪掉，旧 app-switcher/Home projection 在主表 rebuild 前仍可读但必须保持 dirty/stale/pending repair，不能因为 app directory metadata 已更新就被命名为 complete。底层 cleanup 通过 repair-provider `recordAppTerminated(processIdentifier:)` 边界提交，service 不再直接调用 coordinator cancel / WindowRecord clear / AX live registry remove；repair-provider 内部负责取消 pending repair、移除 terminated PID 的 WindowRecord mapping state 与 AX live registry。正常 service termination path 现在只在 runtime maintenance queue 内调用 `RuntimeReadModelStore.markAppTerminatedForMainTableProjection(...)` 维护 app directory/current-app/dirty metadata，caller thread 只发送 lifecycle signal；该 metadata path 不剪枝或重写 `committedSearchIndex`。cleanup 后再通过 `RuntimeMainTableProjectionBuilding.appSwitcherProjectionPayloadFromMainTables(...)` 从剩余 app directory + WindowRecord 主表预提交 app-switcher/Home summary projection cache，coverage guard 允许被 terminated appID 缺席但仍要求其它既有 appID 被主表覆盖；该主表 commit 成功后才会清理对应 app termination dirty scope，让 app-switcher/Home projection 进入当前主表 complete 状态。此时 Search 只能返回 last committed index 的 `degradedStaleCommittedResult` / stale committed read 与 dirty/freshness metadata，旧 index 里可以仍含 terminated app/window row，不能命名为 fresh、complete、latest、current-generation committed 或最新完整结果。旧的 `RuntimeReadModelStore.markAppTerminated(...)` direct-call 同步 projection/Search 剪枝入口已删除；termination 不能再通过 store-local sanitization、legacy projection PID inference 或 queue 外同步 metadata mutation 冒充 freshness barrier 成功 proof。
...
- Search surface 在 `missingCommittedIndex` 时也只读 committed-index contract 并发送 bounded freshness barrier request：`RuntimeSearchIndexRead.shouldRequestFreshnessBarrier` 覆盖 missing 与 degraded/stale 两种非 current-generation state，`LiveSwitcherModel` 缺 committed projection 时不会从 session apps rebuild Search，也不会进入 active Search 或展示 partial results；它只记录 missing diagnostic 并请求 runtime maintenance 尝试提交新 generation。barrier 成功前该状态仍是 missing committed index，不是 fresh、complete、latest/current-generation 或最新完整搜索结果。
...
- Phase 6 本轮 P0 继续把 app directory 从 repair/fallback helper 提升为 runtime maintenance fact source：新增 `RuntimeAppDirectoryProviding` / `RuntimeWorkspaceAppDirectoryProvider`，默认 `RuntimeProjectionService` 在 app-switcher maintenance 与 bounded Search freshness barrier 前先用 public `NSWorkspace.runningApplications` 过滤结果写入 `RuntimeReadModelStore.commitAppDirectoryProviderEvidence(...)`，再通过 `RuntimeMainTableProjectionBuilding.appSwitcherProjectionPayloadFromMainTables(...)` 从 app directory + WindowRecord 主表生成 app-switcher/Home projection cache。custom injected repair-provider 入口默认不启用 workspace provider，避免迁移测试/compat bridge 被隐式 running-app state 覆盖；需要长期 directory evidence 的 service path 必须显式注入 provider 或使用默认 production service。Required behavior coverage: `testRuntimeProjectionServiceCommitsAppDirectoryProviderProjectionFromMainTablesAsStaleWithFullRepairFallback` 证明没有 full-repair payload 时 provider evidence 可补齐 app directory，projection windows 来自 WindowRecord 主表；若 projection 仍带 dirty/pending metadata，它只能是 degraded/stale committed read，并会安排 low-priority full repair fallback 补事实。Search readback 仍是 last committed index 的 `degradedStaleCommittedResult`，即使 projection cache 已可读，也不能叫 fresh、complete、latest、current-generation committed 或最新完整搜索结果态，除非 bounded freshness barrier 成功提交新 generation。P1 同轮更新 coverage matrix；P2 不新增 UI/E2E 或 pressure proof，因为 surface call sequence、activation oracle、Search barrier bound、CG/AX/Space sampling cadence 与 repeated hot-path shape 未改变。Remaining gaps: 真实 committed-index Search UI proof、真实 topology UI/pressure proof 继续跟踪。
...
- Phase 6 本轮 P0 继续守住 app directory rank 在 scoped repair / lifecycle upsert 下的 ownership：focused current-app repair evidence 现在从 `RuntimeWindowListFacts.rankByPID` 写入 `RuntimeCurrentAppRepairEvidence.appDirectoryEntries`，`RuntimeCurrentAppWindowProjectionAssemblyInput(...)` 的 direct assembly 也保留传入 rank；当 current-app repair 或 lifecycle upsert 提交缺失 rank 的 entry 时，`RuntimeAppDirectoryState.upsert(...)` 会保留同 PID committed rank，避免 scoped repair fallback 把 Runtime 长期 rank 主表降级成无 rank evidence。显式新 rank 仍按 app directory equality 推进 `appLifecycle` generation。Required behavior coverage: `testRuntimeReadModelStoreCurrentAppRepairEvidencePreservesExistingActivationRank` 证明 unranked repair 不推进 generation、不擦掉 rank、不扰乱 app ordering，而 ranked repair 会推进 generation 并重排 projection；targeted `run-flowtabtests-local.sh` variation 已覆盖该 contract。P2 不新增 UI/E2E 或 pressure proof，因为 surface sequence、activation oracle、Search barrier bound、CG/AX/Space sampling cadence 与 repeated hot-path shape 未改变。Search barrier 未成功提交新 generation 时仍只能暴露 missing committed index 或 degraded/stale committed result，不能命名为 fresh、complete、latest/current-generation committed 或最新完整搜索结果态。
...
- Phase 6 本轮 P0 继续补齐 Search missing committed-index signal：`RuntimeSearchIndexRead.shouldRequestFreshnessBarrier` 现在对 `missingCommittedIndex` 与 `degradedStaleCommitted` 都返回 true；`LiveSwitcherModel.rebuildSearchIndexFromCommittedProjection(...)` 在缺 committed projection 时只记录 missing diagnostic、请求 `.searchFreshnessBarrier`，并返回 false，不从当前 session apps/windows rebuild Search index、不进入 Search active、不展示 partial result。Required behavior coverage 扩展 `testLiveSwitcherModelSearchDoesNotFallbackToSessionAppsWithoutCommittedIndex`，证明 session app contamination 不会成为 Search 结果，missing state 会发送 bounded barrier request。P2 不新增 UI/E2E 或 pressure proof，因为 visible Search flow、activation oracle、barrier bound、sampling cadence 与 repeated query algorithm 未改变。barrier 成功提交新 generation 前，该状态仍只能是 missing committed index，不能命名为 fresh、complete、latest/current-generation 或最新完整搜索结果。
- Phase 6 本轮 P0 继续删除 Home surface-local app directory topology state：`HomeLandingView` 不再缓存 running-app signature，也不再在 active/appear refresh path 读取 `NSWorkspace.runningApplications` 比较 surface-local app set。refresh gating 被收敛到 `HomeRuntimeProjectionRefreshPolicy`，只根据 Home projection summary 是否存在、initial window-count loading 是否结束来请求 runtime projection refresh；缺 projection 时由 Home runtime reader 发送 shared runtime maintenance/dirty signal 并保留当前 committed UI state。Required unit/behavior coverage: `testHomeRuntimeProjectionRefreshPolicyRequestsOnlyAfterProjectionStateIsLoaded` 证明调度规则只依赖 projection-loaded/loading state，`testHomeRuntimeRefreshReaderSignalsRuntimeRepairWhenProjectionIsMissingWithoutHomeFallback` 继续证明缺 projection 不走 Home fallback sampling。P2 不新增 UI/E2E 或 pressure proof，因为 visible Home flow、activation oracle、Search barrier bound、CG/AX/Space sampling cadence 与 repeated hot-path shape 未改变。Search freshness barrier 未成功提交新 generation 时仍只能暴露 degraded/stale committed result 或 missing committed index，不能命名为 fresh/complete/latest/current-generation 或最新完整搜索结果态。
- Phase 6 本轮 P0 继续补齐 Home detail projection 的 freshness/generation ownership：`RuntimeHomeAppDetailProjection` 现在携带 `RuntimeProjectionFreshness`，`RuntimeReadModelStore.readHomeAppDetailProjection(appID:)` 从 current-app projection cache 派生 detail 时会重新计算当前 dirty/pending metadata 并传递给 Home detail read。这样 Home detail read 与 app-switcher/Home summary/current-app/Search projection 一样有 generation/freshness 边界；dirty app-window state 下仍可返回 last committed Home detail，但必须标记 stale/degraded，不再是无 freshness payload。Required behavior coverage 扩展 `testRuntimeReadModelStoreOwnsHomeDetailProjectionFromCurrentAppCache`，证明 clean detail 为 complete，随后 app-window dirty signal 下同一 detail read 变为 incomplete，并携带 dirty app/pid/pending scope。P2 不新增 UI/E2E 或 pressure proof，因为 visible Home flow、activation oracle、Search barrier bound、CG/AX/Space sampling cadence 与 repeated hot-path shape 未改变。Search freshness barrier 未成功提交新 generation 时仍只能暴露 degraded/stale committed result 或 missing committed index，不能命名为 fresh/complete/latest/current-generation 或最新完整搜索结果态。
- Phase 6 本轮 P0 收紧 Search committed-index freshness 命名 proof：production `RuntimeReadModelStore.readCommittedSearchIndexForSearch()` 已经只在 committed index 覆盖当前 generation 且无 dirty 时返回 `committedGenerationValidated` / `committedGenerationResult`；本轮把 `testRuntimeReadModelStoreKeepsCommittedSearchStaleWhenProjectionCommitClearsDirtyWithoutSearchGeneration` 扩展为 raw label contract，证明 current-app projection repair 即使清掉 dirty，也不能把未通过 freshness barrier 提交新 generation 的 last committed index 命名为 fresh、complete 或 latest。当前行为必须是 `degradedStaleCommitted` / `degradedStaleCommittedResult`，并携带旧 committed index 的 source generation。P1 同轮更新 matrix；P2 不新增 UI/E2E 或 pressure proof，因为 Search surface call sequence、barrier bound、index rebuild 算法、activation oracle 与 CG/AX/Space sampling cadence 未改变。
- Phase 6 本轮 P0 收紧 app termination projection freshness ownership：`RuntimeReadModelStore.markAppTerminatedForMainTableProjection(...)` 在 app directory state 已初始化时只用 app directory entries 判断 terminated PID，未知或已删除 PID 不再从 legacy app-switcher/Home/Search cache 推断；last-PID termination 的旧 app-switcher/Home projection 在主表 rebuild 前保持 dirty/stale/pending `appTerminated:<appID>`，不能标记 complete。`RuntimeProjectionService.signalAppTerminated(...)` 在 repair-provider cleanup 与 app-directory provider evidence 后，通过 `RuntimeMainTableProjectionBuilding.appSwitcherProjectionPayloadFromMainTables(...)` 成功重建主表 app-switcher/Home projection 时，才清理对应 app termination dirty scope，使 rebuilt projection complete。Required behavior coverage 更新 store termination、same-app multi-PID termination、service main-table termination rebuild 与 service same-app pruning tests，证明 store metadata state stale、duplicate/unknown PID 不推进 generation、service rebuild 后 projection complete，而 Search 仍是 degraded/stale committed。P2 不新增 UI/E2E 或 pressure proof，因为 surface read APIs、activation oracle、Search barrier bound、sampling cadence 与 repeated hot-path cost 未改变；真实 committed-index Search UI proof 和更广 topology UI/pressure proof 继续作为 gap。
- Phase 6 本轮 P0 收紧 app-directory-derived projection freshness ownership：`RuntimeReadModelStore.readAppSwitcherProjection()` / `readHomeSummaryProjection()` 现在区分 committed projection cache 与 app-directory-derived fallback。committed cache 仍按当前 dirty metadata 重新计算 complete；但 derived fallback 即使 app directory 主表 clean，也固定返回 degraded/stale freshness，因为它只有 app identity/order 与 zero-window summary，没有成功提交 app-switcher/Home 主表 projection generation、WindowRecord rows 或 contexts。`RuntimeProjectionService` 在 provider evidence 写入 app directory 但 main-table builder 没有提交 payload 时，也只能暴露 degraded app-only projection 和 missing committed Search index，不能叫 fresh、complete、latest 或 current-generation Search/result state。Required behavior coverage 扩展 `testRuntimeReadModelStoreAppDirectoryProjectionUsesActivationRankEvidence` 与新增 `testRuntimeProjectionServiceKeepsAppDirectoryDerivedProjectionDegradedWithoutMainTableCommit`；P2 不新增 UI/E2E 或 pressure proof，因为 surface read APIs、activation oracle、Search barrier bound、sampling cadence 与 repeated hot-path cost 未改变。
- Phase 6 本轮 P0 继续收紧 main-table app-switcher/Home payload coverage ownership：`RuntimeAppSwitcherProjectionPayload` 现在携带 `hasCompleteWindowCoverage`，`RuntimeMainTableProjectionBuilder` 只有在每个 projected app 都有 WindowRecord-derived window rows 与 matching `RuntimeAppContext.windowsByID` 时才标记 complete。`RuntimeReadModelStore.commitMainTableAppSwitcherProjectionPayload(...)` 与 subsequent read revalidation 都会保留该 incomplete coverage bit；dirty 清空不能把 incomplete projection cache 提升为 complete。Search freshness barrier 现在也要求 committed projection cache complete，app-directory-only / no-AX zero-window payload 只能保持 missing committed index 或 stale/degraded committed read，不能提交 latest/current-generation Search index。Required behavior coverage: `testRuntimeProjectionServiceKeepsAppDirectoryOnlyMainTableProjectionIncompleteForSearch` 证明 provider evidence + empty WindowRecord main-table payload 可读但 incomplete、安排 fallback 且 barrier 不提交 Search；`testRuntimeProjectionRepairProviderRealPathWithoutAccessibilityBuildsMainTableProjectionPayload` 证明 no-AX real path payload 明确不是 complete coverage；provider-with-WindowRecord stale fallback 与 repaired Search generation tests 证明 dirty/incomplete coverage 保持 stale，而完整主表 coverage 仍可提交。P2 不新增 UI/E2E 或 pressure proof，因为 surface read APIs、activation oracle、sampling cadence 与 repeated hot-path cost 未改变。
- Phase 6 本轮 P0 继续把 readable projection 与 complete projection 的 ownership 语义拆开：`RuntimeReadModelDiagnostics` 新增 complete app-switcher/Home summary projection 标志，只在 committed projection cache 本身 complete 且当前无 dirty/pending metadata 时为 true；app-directory-derived fallback 或 WindowRecord/context 覆盖不足的 main-table projection 仍可读，但 diagnostics 不再把它们报告成 complete。`RuntimeProjectionService.requestAppSwitcherProjectionMaintenance(...)` 因此只在 complete projection 已存在时跳过 full-repair fallback；derived 或 incomplete projection 会保留 degraded/stale committed read，并安排 low-priority full repair fallback 补事实。Required behavior coverage 更新 app-directory-derived 与 app-directory-only main-table tests，证明这两类状态不会进入 fresh/complete Search 结果态，只能保持 missing committed index 或 degraded/stale committed result；P2 不新增 UI/E2E 或 pressure proof，因为 surface read APIs、activation oracle、Search barrier bound、sampling cadence 与 repeated hot-path cost 未改变。
