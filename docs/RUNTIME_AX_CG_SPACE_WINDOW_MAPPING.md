...
- [ ] 公开唯一匹配已使用标题、frame、focused/main 前台候选与 minimized/offscreen 候选作为 public assignment 规则。代表性 edge workflow 已用真实重复同标题窗口证明 public AX state tie-breaker 会在多候选 AX/CG 拓扑中触发；同一 fixture 现在也通过真实 minimized startup state 证明 AX 可读到 minimized public state、CG 对应窗口处于 offscreen，并最终进入 `window-entries ... off:minimized=1` 输出。当前仍未完成的是更广 focused/main 状态排列、minimized public-state tie-breaker 的真实触发证明，以及多显示器/fullscreen 组合下的 UI/E2E 覆盖。此外它们只收窄已由标题/frame 建立的候选集合，不替代私有 exact bridge 或 Space topology recovery。
