...
- `RuntimeSnapshotProvider.snapshot()` wrapper 已删除；full repair 现在只通过 repair-provider `fullRepairProjectionPayload()` 枚举 running apps，并调用 `RuntimeFullRepairFactSource.collectWindowFacts(for:)` 采集 cleanup/prune、onscreen/all CG、AX window data 与 rank facts。该路径性质是 scheduler repair/fallback，不是 hot-path read。
...
- Phase 5 本轮 P0 继续收窄 full-repair fact-source ownership：`RuntimeFullRepairFactSource` 现在承载 full-repair cleanup/prune、onscreen/all CG collection、AX window data collection 与 app rank facts；`RuntimeProjectionRepairProvider+ProjectionBuilders.swift` 只消费 `RuntimeFullRepairWindowFacts` 来做 app directory selection、projection seed 与 payload assembly。该拆分不改变 full repair 的 repair/fallback/cold-start 性质，不新增 hot-path read，也不把 Search barrier 成功 oracle 交给 full repair。
