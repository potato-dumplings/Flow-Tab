...
- `FlowTabTests.testRuntimeSnapshotProviderUsesUITestMockDatasetWhenLaunchFlagEnabled` 证明 provider-side `fullRepairProjectionPayload()` 可直接产出 app/context projection payload，`snapshot()` 只包装同一 payload；`testRuntimeProjectionServiceDefaultFullRepairCommitsProviderProjectionPayload` 证明默认 full repair executor 不经 injected snapshot result 也能提交 provider projection payload。
