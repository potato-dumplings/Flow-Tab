...
- Phase 5 本轮 P0 继续收窄 runtime maintenance execution ownership：`RuntimeProjectionReconciliationExecutionOutcome`、`RuntimeProjectionReconciliationDrainResult`、`RuntimeFullRepairProjectionCommitSummary`、`RuntimeProjectionReconciliationExecutor` 与默认 repair executor 已从 `RuntimeProjectionService.swift` 抽到 `RuntimeProjectionMaintenance.swift`。service 文件只消费 maintenance execution boundary，并继续拥有 feature-facing `RuntimeProjectionServing`、read-model store commits、maintenance queue drain、Search staging/commit 条件与 freshness policy；repair-provider fact source 与 coordinator mutation 仍不回流到 service。Search freshness contract 不变，bounded barrier 成功提交新 generation 之前，当前 Search 行为只能暴露为 degraded/stale committed result 与 dirty/freshness metadata。
