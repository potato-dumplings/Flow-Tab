...
- scoped repair affected-target derivation 仍由 `RuntimeProjectionRepairProvider+Reconciliation.swift` 组装；`RuntimeAppWindowReconciliationResult` 现在定义在 `RuntimeProjectionRepairProvider.swift`，与返回它的 `RuntimeProjectionRepairProviding` contract 同层。`RuntimeSnapshotProvider+Reconciliation.swift` 已删除，Space topology / verified focus / AX destroyed evidence 写回现在只通过 `RuntimeWindowRecordStore` semantic methods 进入；raw table mutation helper 已降为 store 文件内 private implementation detail，provider 与 repair-provider 不再传入 WindowRecord mapping state。
