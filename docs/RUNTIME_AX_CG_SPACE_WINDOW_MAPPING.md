...
- Search staging 的 scoped repair 输入由 `RuntimeReadModelStore` 消费 repaired current-app projection payload；staging 与“只有本轮 staged payload 且没有 deferred/pending repair 时才提交 committed generation”的判定由 `commitSearchFreshnessBarrierPayloads(...)` 在 store transaction 内完成。`RuntimeProjectionService` 只负责 barrier drain 并提交 drain facts，不再手动从 repair/current-app payload 中挑 app candidate、维护 staging index 或拼接 staging->committed 条件；裸 `commitStagedSearchIndex(...)` 外部入口已删除，避免任何 caller 绕过 freshness barrier 把 staging 提升为正常结果态。
...
- Phase 5 本轮 P0 继续收窄 Search freshness-barrier commit ownership：`RuntimeProjectionService` 不再把 raw `RuntimeReconciliationReason.searchFreshnessBarrier` 传给 repair-provider，也不再直接调用 `stageSearchIndexCurrentAppWindowPayloads(...)` 后自行拼 `deferred == 0 && !pending` 的 staging->committed 条件；service 只调用 `RuntimeProjectionRepairProvider.promoteSearchFreshnessBarrierRequests(now:)`、bounded drain ready repairs，然后把 drain facts 交给 `RuntimeReadModelStore.commitSearchFreshnessBarrierPayloads(...)`。store 在同一 transaction 内负责 staging current repaired payload，并且只有本轮 staged payload、无 deferred repair、无 pending repair 时才原子提交新 committed generation；裸 `commitStagedSearchIndex(...)` 外部入口已删除，`commitStagedSearchIndexLocked(...)` 只作为 store-private implementation detail 存在。barrier 未成功提交新 generation 时，当前 Search 行为只能暴露为 degraded/stale committed result，不能命名为 fresh、complete、latest 或 current-generation committed。
