...
- `RuntimeFullRepairProjectionAssembler` 省略 `appDirectoryEntries` 时也不再从 current-app payloads / projection inputs 合成 directory evidence；assembler 只排序 app rows 与 contexts，完整 app directory 必须由 full repair fact collection 显式传入。
