...
- current-app repair drain 现在也走同一主表 projection builder，并且 drainer/service 边界不再运输 repaired `RuntimeCurrentAppWindowPayload` projection rows：`RuntimeProjectionReconciliationDrainer` 把 app/space repair result 降级为 `RuntimeCurrentAppRepairEvidence`，只携带 appID、pid、app directory entries 与 transient-empty evidence。`RuntimeProjectionService` 只 upsert 这份 directory evidence，然后用 evidence 的 appID/pid 调用 `currentAppWindowPayloadFromMainTables(...)` 从 app directory + WindowRecordStore 生成 current-app/app-switcher/Home summary projection cache。repaired payload 的 candidate/window rows 不能穿过 drain/service 边界刷新正常 projection cache 或 Search freshness；Search barrier 成功态只能来自随后覆盖当前 generation 的 projection cache commit。
