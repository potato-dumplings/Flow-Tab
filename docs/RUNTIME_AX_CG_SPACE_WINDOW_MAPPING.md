...
- 已知 `AXWindowID` 的 destroyed 信号可通过 shared `RuntimeSnapshotService.signalAXWindowDestroyed(appID:pid:axWindowID:)` 进入 runtime：`RuntimeWindowRecord` 会清除 current AX attachment、移除当前 `AX -> CG` / `CG -> AX` 派生索引、保留历史 exact 作为 sticky 证据，并标记该 record 需要 reconciliation；Home `AXObserver` destroyed callback 若携带可由 `AXLiveWindowRegistry` 识别的既有 window element，也会走该 shared runtime 入口，解析不到时只退回普通 app dirty。
