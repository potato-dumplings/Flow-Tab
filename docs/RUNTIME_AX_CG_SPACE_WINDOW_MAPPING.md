...
- Phase 5 本轮 P0 继续收窄 provider fact-collection diagnostics vocabulary：`RuntimeSnapshotProvider+Diagnostics.swift` 的 timing helper 已从 `logSnapshotTiming` / `snapshotTimingLine` / `formatSnapshotMilliseconds` 改为 `logRuntimeFactTiming` / `runtimeFactTimingLine` / `formatRuntimeFactMilliseconds`。底层日志仍写 `.snapshot` category，事件名和字段保持不变，因为这里记录的是 CG/AX/Space fact collection diagnostics；但 provider 内部 API 不再把这些 helper 表达成 surface 可读的 snapshot-shaped timing seam。这不改变采样、projection commit、activation、Search 或 hot-path behavior。Required behavior coverage: `testRuntimeSnapshotProviderWindowEntriesCarrySpaceEvidence` 与 `testRuntimeSnapshotProviderWindowListFiltersDuplicateFullscreenGeometryHosts` 已通过 `run-flowtabtests-local.sh` targeted run，并重新编译 `RuntimeSnapshotProvider.swift` / `RuntimeSnapshotProvider+Diagnostics.swift`。freshness barrier 未成功提交新 generation 时 Search 仍只能是 degraded/stale committed result。
