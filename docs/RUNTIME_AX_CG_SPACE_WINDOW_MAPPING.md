...
- provider full repair builder 现在也通过 `WindowListEntry.projectionSeed(lastActiveAt:)` 进入 `RuntimeCurrentAppWindowPayload` 组装 app-switcher candidate 与 `RuntimeAppContext`，不再在 full repair loop 内手写另一套 candidate/context projection assembly；`fullRepairProjectionPayload()`、其 private full-repair `collectWindowData(for:)` 聚合入口与 `FullRepairProjectionAssembly*` deterministic seam 已迁入 `RuntimeSnapshotProvider+AppProjectionBuilders.swift`，让 provider core 文件只保留底层 CG/AX/Space 采样与窗口事实基础设施。
