...
- Preview paging/session-pinning behavior tests now use the shared `makeAppSwitcherProjectionModel` helper with a `runtimeProjectionService` return label and assert app-switcher projection reads plus `.switcherSessionStarted` maintenance requests. The `WindowPreviewSessionPinning` coverage no longer treats dead full/lightweight snapshot request counters as service ownership proof; provider resolver cleanup remains a separate old-counter gap.
