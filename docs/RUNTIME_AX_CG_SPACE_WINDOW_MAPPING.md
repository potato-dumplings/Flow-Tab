...
- `RuntimeReadModelStore` 与 projection cache 的 P0 边界已落地；Home surface state/API 已把 selected-app detail cache 和 API payload 类型迁移为 `RuntimeHomeAppDetailProjection`，不再把 projection read boundary 表达成 Home snapshot cache；仍需把 projection builders 从旧 snapshot 兼容桥迁移到底层主表生成，并拆掉 provider repair 侧的 `RuntimeHomeAppSnapshot` 兼容 payload。
