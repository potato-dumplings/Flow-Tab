...
- `RuntimeAppWindowRepairPayload` 只保留为 app-window repair 兼容 wrapper，由 `RuntimeCurrentAppWindowPayload` 包装生成；其 seed assembly initializer 已删除，provider 只把采样事实转换为 `RuntimeAppWindowProjectionSeed`，不再私有维护 candidate/context/summary projection assembly，也不再把 repair payload 当作 current-app projection 的上游。

但当前实现仍有迁移对象：

- service 层 `RuntimeProjectionService.fallbackRuntimeSnapshot()` full snapshot bridge 与 concrete-only `fallbackLightweightAppSnapshot()` lightweight bridge 均已删除；`RuntimeProjectionServing` 已不再暴露 full snapshot bridge、同步 lightweight bridge 或 `currentCGWindowsByPID()` live CG z-order read，Switcher/Home 的 P0 首读路径已优先读取 projection，`Option+Tab` 缺 app-switcher projection 时只请求 shared runtime maintenance，不再同步调用 lightweight snapshot bridge；`Control+Tab` 缺 current-app projection 时只发送 runtime dirty/repair signal，不再同步调用 focused snapshot bridge。service-facing focused snapshot 兼容入口已删除，provider 内部 app-local reconciliation pullback 现在直接返回 `RuntimeCurrentAppWindowPayload`；provider-facing `appWindowRepairPayload` / `focusedAppWindowRepairPayload` 兼容 API 已删除，`RuntimeAppWindowRepairPayload` 仅是类型级迁移兼容包装。
...
- full repair 在 `RuntimeProjectionService` 的 drain/outcome 边界已改为 `RuntimeFullRepairProjectionPayload`，service 不再把 `RuntimeSnapshot` 作为 in-flight repair result 保存或提交；默认 executor 直接调用 provider-side `fullRepairProjectionPayload()` builder，`RuntimeSnapshotProvider.snapshot()` 只作为迁移兼容包装从该 payload 构造旧 `RuntimeSnapshot`。
...
- service 层 feature-facing full snapshot fallback 已从 `RuntimeProjectionService` 删除；full repair outcome 现在也只携带 `RuntimeFullRepairProjectionPayload`。full repair builder 已命名为 provider-side `fullRepairProjectionPayload()`，性质是 low-priority repair/diagnostic 或 cold-start 输入；`RuntimeSnapshotProvider.snapshot()` 只保留为迁移兼容包装，不是 Switcher/Home/Search hot-path read API，也不是 Search freshness barrier 的成功 oracle。
...
- `FlowTabTests.testRuntimeSnapshotProviderUsesProjectionPayloadForUITestMockDatasetWhenLaunchFlagEnabled` 证明 provider-side `fullRepairProjectionPayload()` 可直接产出 app/context projection payload，`snapshot()` 只保留为包装同一 payload 的迁移兼容断言；`testRuntimeProjectionServiceDefaultFullRepairCommitsProviderProjectionPayload` 证明默认 full repair executor 不经 injected snapshot result 也能提交 provider projection payload。
...
- `RuntimeReadModelStore` 与 projection cache 的 P0 边界已落地；Home surface state/API 已把 selected-app detail cache 和 API payload 类型迁移为 `RuntimeHomeAppDetailProjection`，不再把 projection read boundary 表达成 Home snapshot cache；app identity 规则已收敛到 runtime-owned `RuntimeAppIdentity`，AppDelegate lifecycle signal 与 Switcher focused-current-app projection read 不再向 `RuntimeSnapshotProvider` 查询 appID；provider current-app pullback 已迁到 `currentAppWindowPayload(for:)` / `focusedCurrentAppWindowPayload(processIdentifier:)`，provider-facing `appWindowRepairPayload` / `focusedAppWindowRepairPayload` 兼容 API 已删除；provider summary compatibility APIs `homeSummaryProjections()` / `homeSummaryProjection(for:)` 已删除，Home summary 只能来自 `RuntimeReadModelStore` projection 或 current-app payload 的 summary fact；承载这些 provider 兼容 projection/repair builders 的文件已命名为 `RuntimeSnapshotProvider+AppProjectionBuilders.swift`，不再保留 `HomeApps` 文件边界；current-app projection payload 已拥有 projection seed 到 summary/candidate/context 的 assembly，`RuntimeAppWindowRepairPayload` 的 seed assembly initializer 已删除并只保留 `currentAppWindowPayload` 兼容包装，full repair payload 已收敛到 provider-side `fullRepairProjectionPayload()` 与 shared `RuntimeFullRepairProjectionPayload`，deterministic assembly testing seam 也已迁到 `FullRepairProjectionAssembly*`，而不是 `RuntimeProjectionService` 或 provider testing API 持有 `RuntimeSnapshot` result；provider 只负责提供采样事实 seed；current-app payload 提交时会由 store 以既有 projection 为 base 同步 upsert current-app、app-switcher 与 Home summary projection；UI-test runtime dataset 也只维护 app-switcher projection seed、contexts 与 current-app payloads，test launch option 内部 API 使用 projection 命名，full `RuntimeSnapshot` 只留在 provider `snapshot()` 迁移兼容包装。仍需把 projection builders 从旧 full repair 兼容桥迁移到底层主表生成。
