...
- Phase 6 本轮 P0 继续收紧 Search freshness 命名：`RuntimeReadModelStore.commitAppSwitcherProjection(...)` 不再在 `clearsDirtyState` 时隐式构建 `committedSearchIndex`，普通 app-switcher/Home projection commit 因此不能让 Search 进入 fresh/complete/latest/current-generation 结果态。`markAppTerminated(...)` 对已有 committed Search index 只做 degraded/stale sanitization：可以剪掉已退出 app entry，但保留旧 `sourceGeneration`，所以 barrier 未成功提交新 generation 前 readback 仍是 `degradedStaleCommittedResult`。P1 同轮把既有 tests 的 committed Search fixture seed 改为显式 `commitSearchFreshnessBarrierFromProjectionCache(...)`，并更新文档/矩阵；P2 不新增 UI/E2E 或 pressure proof，因为 surface call sequence、barrier bound、sampling cadence、activation oracle 与 repeated hot-path cost 未改变。
