...
- `RuntimeWindowRecord` 现在也拥有 window-layer CG fact 派生：`windowLayerCGWindows(...)` 把 live valid CG facts 与 record-synthesized known-CG evidence 合并，并保持 live facts 优先、stale synthesized evidence 只作为补充。provider window mapping resolution 只消费这个 WindowRecord-owned 派生，不再自己拼接 live/synthesized CG fact list。
