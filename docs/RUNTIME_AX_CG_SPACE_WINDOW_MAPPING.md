...
- [ ] sticky 与 `space-backed` 条目的缺证删除已经具备 record-level grace，但当前 window-layer 展示仍主要来自当前 valid CG 集合，尚未把缺席但仍在 grace 内、且具有真实恢复路径的条目完整输出到用户可提交路径。
