...
- app directory state 现在只接受显式 app directory evidence：`RuntimeReadModelStore.commitAppSwitcherProjection(...)` 省略 `appDirectoryEntries` 时不再从 `contextsByID` 合成或替换 directory；`RuntimeFullRepairProjectionPayload` 则必须显式传入 full repair directory entries 或显式空 evidence，不能从 contexts 反推。full repair / current-app repair / lifecycle signal 必须显式携带 directory evidence 才能通过 `RuntimeAppDirectoryState` 更新 directory state。
