...
- `RuntimeWindowRecord` 的 CG state refresh、exact-match CG attachment 与 synthesized known-CG evidence API 现在直接使用 top-level `RuntimeCGWindowEntry`；provider-nested `RuntimeSnapshotProvider.CGWindowEntry` 只作为迁移期 typealias 留在底层 fact-source 实现，长期 CG-first 主表不再把 CG fact ownership 命名给 provider。
