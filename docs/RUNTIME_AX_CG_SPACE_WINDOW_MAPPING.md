...
- [ ] 公开唯一匹配当前主要依赖标题与 frame；文档要求的 focused、main、最小化状态等公开信号尚未系统纳入统一匹配管线。
