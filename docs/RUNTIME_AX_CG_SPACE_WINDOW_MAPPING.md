...
- `RuntimeCurrentAppWindowPayload` 现在拥有 app-window projection seed 到 Home summary、app-switcher candidate、`RuntimeAppContext` 的组装规则；repair-provider 的 appID-scoped `currentAppWindowPayload(for:)` 仍作为 UI-test/fixture 与 repair/fact bridge 产出 scoped payload，但 focused-PID scoped repair 已删除 `focusedCurrentAppWindowPayload(processIdentifier:)` payload helper，`RuntimeAppWindowReconciliationResult` 只通过 `focusedCurrentAppRepairEvidence(...)` 暴露 `RuntimeCurrentAppRepairEvidence`。正常 projection cache 重新由 `currentAppWindowPayloadFromMainTables(...)` 生成，不再把 repaired payload candidate/window rows 当 normal projection source。
...
- Phase 5 本轮 P0 收窄 current-app projection payload 派生入口：`RuntimeCurrentAppWindowPayload` 已删除 `app` / `appGroup` / rank convenience initializer，并把 seed-to-summary/candidate/context initializer 降为 private；当时 repair-provider `currentAppWindowPayload(for:)` 与 `focusedCurrentAppWindowPayload(processIdentifier:)` 显式提交 `RuntimeCurrentAppWindowProjectionAssemblyInput`。2026-06-24 Phase 6 后 focused-PID scoped repair 已改为 evidence-only，不再保留 focused payload helper。这只是 projection payload ownership cleanup，不改变 Search freshness contract；barrier 未成功提交新 generation 时仍必须暴露为 degraded/stale committed result，而不能命名为 fresh、complete、latest 或 current-generation result。
...
- Phase 5 本轮 P0 继续收窄 full-repair commit/freshness ownership：`RuntimeReadModelStore.commitFullRepairProjectionPayload(...)` 现在拥有 full-repair cold-start 与 dirty degraded fallback 的原子提交规则，并返回 `RuntimeFullRepairProjectionCommitSummary` 供 service 诊断汇总。cold-start full repair 只有在没有既有 app-switcher projection 且没有 dirty/pending metadata 时才会提交 clean projection/app-directory evidence；它不能直接提交 verified current-generation committed Search index。dirty full-repair fallback 只能提交 degraded app-switcher projection，保留 dirty/freshness metadata、staging search index 与 last committed Search index。freshness barrier 成功提交新 generation 前，当前 Search 行为必须继续写成 missing committed index 或 degraded/stale committed result，不能命名为 fresh、complete、latest 或 current-generation committed。
