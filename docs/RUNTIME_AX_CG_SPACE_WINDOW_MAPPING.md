...
- provider full repair builder 现在也通过 `WindowListEntry.projectionSeed(lastActiveAt:)` 进入 `RuntimeCurrentAppWindowPayload` 组装 app-switcher candidate 与 `RuntimeAppContext`，不再在 full repair loop 内手写另一套 candidate/context projection assembly；`fullRepairProjectionPayload()` 与 `FullRepairProjectionAssembly*` deterministic seam 已迁入 `RuntimeSnapshotProvider+AppProjectionBuilders.swift`，让 provider core 文件只保留采样/窗口事实基础设施。
...
- `RuntimeSnapshotProvider.snapshot()` wrapper 已删除；provider full repair 现在只通过 `fullRepairProjectionPayload()` 枚举 running apps 并进入 `collectWindowData(for:)`，其内部会取 onscreen/all CG 和 AX window data。该路径性质是 scheduler repair/fallback，不是 hot-path read。
