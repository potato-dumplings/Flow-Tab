...
- full repair fallback 的 target / high-priority cancellation 已落到 scheduler；完整 backoff policy、更细粒度 facts 拆分与多轮 scoped repair 失败后的自动升级仍需扩展。
...
- 本轮 targeted `FlowTabPriorityCoverageTests` 新增 full repair / Search barrier 3 个用例通过；类级复跑另有非本阶段 `testSwitcherPanelControllerRecoverableOcclusionKeepsSessionVisible` visibility diagnostic 断言失败，未作为本阶段 runtime ownership blocker。
- P2 待补：完整 full repair fallback backoff/升级策略、真实 topology UI/E2E 与 pressure proof。
...
- Switcher session-start background full snapshot 已降级为 runtime-owned maintenance request；scheduler priority/coalescing/promoted-backoff P1 已落地，Search freshness barrier priority 与 selected/current app-window priority 已进入 runtime coordinator，full repair fallback target / low-priority scheduling / high-priority scoped cancellation 已建模。完整 full repair backoff/自动升级与 full snapshot facts 拆分仍需补齐。
