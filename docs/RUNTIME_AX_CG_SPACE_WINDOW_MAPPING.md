...
- current-app repair drain 现在也走同一主表 projection builder：`RuntimeProjectionService` 对 repaired current-app payload 只 upsert 其 `appDirectoryEntries` evidence，然后用 payload 的 appID/pid 调用 `currentAppWindowPayloadFromMainTables(...)` 从 app directory + WindowRecordStore 生成 current-app/app-switcher/Home summary projection cache。repaired payload 的 candidate/window rows 不能直接刷新正常 projection cache 或 Search freshness；Search barrier 成功态只能来自随后覆盖当前 generation 的 projection cache commit。
