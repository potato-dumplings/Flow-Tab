...
- scoped repair affected-target derivation 仍由 `RuntimeProjectionRepairProvider+Reconciliation.swift` 组装；`RuntimeAppWindowReconciliationResult` 现在定义在 `RuntimeProjectionRepairProvider.swift`，与返回它的 `RuntimeProjectionRepairProviding` contract 同层。`RuntimeSnapshotProvider+Reconciliation.swift` 已删除，Space topology / verified focus / AX destroyed evidence 写回改由 `RuntimeWindowRecordEvidence.swift` 承载，provider 与 repair-provider 只传入 WindowRecord mapping state 和 coordinator。
...
- Phase 5 本轮 P0 继续收窄 WindowRecord affected-target scope ownership：`RuntimeWindowMappingState.affectedCGWindowIDsByPID(...)` 现在拥有 scoped repair affected CGWindowID 从 current CG facts 与 WindowRecord history 合并成 pid->affected-window scope 的规则；`RuntimeProjectionRepairFactSource.collectSpaceTopologyReconciliationTargets(...)` 消费该 WindowRecord-owned scope 组装 `RuntimeSpaceTopologyReconciliationTarget`，`RuntimeProjectionRepairProvider+Reconciliation.swift` 只消费 fact-source target，不再内联 current-CG/window-record 双路径分组，也不再把 app identity pullback 放在 topology target seam。当前这仍是 repair-provider reconciliation 输入，不是长期 WindowRecord/app-directory/topology 直接 rebuild projection；Search freshness contract 不变，barrier 未成功提交新 generation 时仍必须暴露为 degraded/stale committed result。
- Phase 5 本轮 P0 继续收窄 repair-provider result-shape ownership：`RuntimeAppWindowReconciliationResult` 已从 `RuntimeProjectionRepairProvider+Reconciliation.swift` 移到 `RuntimeProjectionRepairProvider.swift`，与返回它的 `RuntimeProjectionRepairProviding.reconcileAppWindows(...)` contract 同层；reconciliation extension 只负责组装该 result 与底层 coordinator/fact-source mutation。当前这仍是 repair-provider contract cleanup，不改变 Search freshness contract；barrier 未成功提交新 generation 时仍必须暴露为 degraded/stale committed result，不能命名为 fresh、complete、latest 或 current-generation committed。
