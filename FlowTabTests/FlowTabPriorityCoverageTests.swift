...
enum FlowTabPriorityCoverageWatchdogPolicy {
    ...
    static let appDelegateLaunchBootstrapEvidence: TimeInterval = 4
}
...
extension FlowTabPriorityCoverageTests {
    ...
    func testPriorityCoverageWatchdogPolicyPreservesSearchComputationPublicationBound() {
        let searchComputationPublication =
            FlowTabPriorityCoverageWatchdogPolicy.searchComputationPublication

        XCTAssertEqual(searchComputationPublication, 1)
        XCTAssertTrue(searchComputationPublication.isFinite)
        XCTAssertGreaterThan(searchComputationPublication, 0)
    }

    func testPriorityCoverageWatchdogPolicyPreservesHomeProjectionEvidenceDeliveryBound() {
        let homeProjectionEvidenceDelivery =
    ...
    func testPriorityCoverageWatchdogPolicyPreservesAppDelegateLaunchBootstrapBound() {
        let appDelegateLaunchBootstrapEvidence =
            FlowTabPriorityCoverageWatchdogPolicy
                .appDelegateLaunchBootstrapEvidence

        XCTAssertEqual(appDelegateLaunchBootstrapEvidence, 4)
        XCTAssertTrue(appDelegateLaunchBootstrapEvidence.isFinite)
        XCTAssertGreaterThan(appDelegateLaunchBootstrapEvidence, 0)
    }
}
