...
extension FlowTabUITests {
    ...
    func testHomePageSingleAppFiveWindowsVariantShowsAllWindowRows() throws {
        let app = makeApp(
            additionalArguments: [
                "--flowtab-ui-reset-defaults",
                "--flowtab-ui-mock-runtime",
        ...
        let expectedWindowIdentifiers = [
            "flowtab.home.window.mock-browser-normal-1",
            "flowtab.home.window.mock-browser-normal-2",
            "flowtab.home.window.mock-browser-fullscreen-1",
            "flowtab.home.window.mock-browser-fullscreen-2",
            "flowtab.home.window.mock-browser-fullscreen-3"
        ]
        for windowIdentifier in expectedWindowIdentifiers {
            let row = app.descendants(matching: .any).matching(identifier: windowIdentifier).firstMatch
            XCTAssertTrue(row.waitForExistence(timeout: 12), "Missing window row: \(windowIdentifier)")
        }
    }

    func testHomePageSingleAppFiveWindowsCGOffSpaceVariantShowsAllWindowRows() throws {
        let app = makeApp(
            additionalArguments: [
                "--flowtab-ui-reset-defaults",
                "--flowtab-ui-mock-runtime",
        ...
        let expectedWindowIdentifiers = [
            "flowtab.home.window.cg-100-240001",
            "flowtab.home.window.cg-100-240002",
            "flowtab.home.window.cg-100-243747",
            "flowtab.home.window.cg-100-243679",
            "flowtab.home.window.cg-100-240029"
        ]
        for windowIdentifier in expectedWindowIdentifiers {
            let row = app.descendants(matching: .any).matching(identifier: windowIdentifier).firstMatch
            XCTAssertTrue(row.waitForExistence(timeout: 12), "Missing window row: \(windowIdentifier)")
    ...
    func testHomePageSingleAppFiveWindowsCGOffSpaceVariantListsStableWindowRowsBeforeRecoveredFullscreenRows() throws {
        let app = makeApp(
            additionalArguments: [
                "--flowtab-ui-reset-defaults",
                "--flowtab-ui-mock-runtime",
        ...
        let firstRecoveredRow = app.descendants(matching: .any)
            .matching(identifier: "flowtab.home.window.cg-100-240001")
            .firstMatch
        let secondRecoveredRow = app.descendants(matching: .any)
            .matching(identifier: "flowtab.home.window.cg-100-240002")
            .firstMatch
        let firstCGRow = app.descendants(matching: .any)
            .matching(identifier: "flowtab.home.window.cg-100-243747")
            .firstMatch

        XCTAssertTrue(firstRecoveredRow.waitForExistence(timeout: 12))
        XCTAssertTrue(secondRecoveredRow.waitForExistence(timeout: 12))
        XCTAssertTrue(firstCGRow.waitForExistence(timeout: 12))
        XCTAssertLessThan(firstRecoveredRow.frame.minY, firstCGRow.frame.minY)
        XCTAssertLessThan(secondRecoveredRow.frame.minY, firstCGRow.frame.minY)
    }

    func testHomePageSingleAppFiveWindowsCGOffSpaceTitledVariantShowsRecoveredTitles() throws {
        let app = makeApp(
            additionalArguments: [
                "--flowtab-ui-reset-defaults",
                "--flowtab-ui-mock-runtime",
        ...
        let expectedTitles = ["Fullscreen 3", "Fullscreen 4", "Fullscreen 5"]
        for title in expectedTitles {
            XCTAssertTrue(app.staticTexts[title].waitForExistence(timeout: 8), "Missing title: \(title)")
        }
        XCTAssertFalse(app.staticTexts["Window #3"].exists)
    }
